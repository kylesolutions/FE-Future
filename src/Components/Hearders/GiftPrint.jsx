...
const BASE_URL = 'http://82.180.146.4:8001';
        ...
        function Mug3D({ mugData, uploadedImage }) {
            ...
            // Find the target mesh for decal application
            useEffect(() => {
                if (scene) {
                    let targetMesh = null;
            ...
            // Load and configure decal texture
            ...
            // Compute bounding box for mug dimensions
            ...
            // Calculate mug dimensions (assuming GLB units are in meters)
            ...
            // Decal placement
            let decalPosition = [0, 0, mugRadius * 1.01]; // Slightly offset from surface
            let decalRotation = [0, 0, 0]; // Align with cylindrical surface
            let decalScale = [0.1, 0.1, 0.1]; // Default scale
            ...
            if (decalTexture && uploadedImage && decalTexture.image && mugHeight > 0 && mugRadius > 0) {
                ...
                const maxDecalWidth = mugRadius * 1.5; // Cover part of the cylindrical surface
                const maxDecalHeight = mugHeight * 0.6; // Limit to 60% of mug height
                ...
                decalPosition = [0, mugHeight * 0.1, mugRadius * 1.01]; // Center vertically, offset radially
                    ...
                    >
                        ...
                        {decalTexture && uploadedImage && meshRef.current && decalTexture.image ? (
                            <Decal
                                ...
                                depthTest={true}
                                depthWrite={false}
                                debug={true} // Enable debug to visualize decal bounds
                            />
                        ) : (
            ...
            useEffect(() => {
                const updateCanvasSize = () => {
                    if (containerRef.current) {
                        ...
                        const newSize = Math.min(containerWidth - 40, window.innerHeight * 0.5, 400);
                                        ...
                                        <Suspense fallback={<div className="text-center"><Loader className="animate-spin" /> <p>Loading 3D Model...</p></div>}>
                                            <Canvas
                                                ...
                                                onContextLost={(e) => {
                                                    ...
                                                    setOrderFeedback({
                                                        type: 'error',
                                                        message: '3D rendering failed due to resource limitations. Try refreshing or using a simpler model.',
                                                    });
                                                }}
                                                onContextRestored={() => {
